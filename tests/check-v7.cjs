...
//   ELECTRON_RUN_AS_NODE=1 "/Applications/Visual Studio Code.app/Contents/MacOS/Code" tests/check-v7.cjs
...
const v7=makeGame('build/v7.html'),{run}=v7;
run('renderTick=()=>{}');

// ---- V6 boots into Easy, which is a world with no national debt at all.
assert.equal(run('S.version'),7);
...
const worldOf=key=>{run(`newGame();applyEra('${key}')`);
 return {gdp:run('S.nations.reduce((a,n)=>a+n.gdp,0)'),pop:run('S.nations.reduce((a,n)=>a+n.pop,0)'),
  trade:run('S.nations.reduce((a,n)=>a+n.policies.trade,0)'),tariff:run('S.nations.reduce((a,n)=>a+n.policies.tariff,0)'),
  frontier:run('worldFrontier()'),pegs:run("S.nations.filter(n=>n.fxRegime==='peg').length")};};
...
assert(w70.pegs>150&&w26.pegs<w70.pegs,'1970 is a world of fixed pegs');
...
v7.scope.legacyJSON=v5.run("S.month=30;JSON.stringify(S)");
run('loadSave(legacyJSON)');
assert.equal(run('S.version'),7);assert.equal(run('S.month'),30);
...
for(const [label,file] of [['V6','build/v6.html'],['V4','build/v4.html'],['V3','src/base/sovereign-v3.html'],['V1','src/base/sovereign-v1.html']]){
 const old=makeGame(file);v7.scope.legacyJSON=old.run('S.month=9;JSON.stringify(S)');
 run('loadSave(legacyJSON)');
 assert.equal(run('S.version'),7,label+' migrates to 7');
...
const api=v7.registry.get('advance_simulation'),month=run('S.month');
api.execute({months:2});assert.equal(run('S.month'),month+2);

console.log(JSON.stringify({status:'PASS',version:7,nations:204,cities:1198,
 defaultMode:'easy',defaultNoDebt:true,
 policies:run('POLICY.length'),programs:run('PROGRAMS.length'),
 fileBytes:Buffer.byteLength(v7.html),lastTickMs:+run('perf').toFixed(2),
 checks:['boots into Easy with debt removed','eras rebuild the world at 1970/1985/2000/2026','pegged eras keep a floating anchor','construction can be skipped','elections, war, lag and volatility rules','generated dispatches are varied and bounded','modes strictly ordered easy→hard',
